...
#
# We download the two variables in sequence, with four years processed concurrently for each. This reduces pressure on the shared Earth Engine project. Download time depends on server traffic and other active sessions.

# %%
from tools.geetools import CMIPDownloader

downloader_t = CMIPDownloader(var='tas', starty=1979, endy=2100, shape=catchment, processes=4, dir=cmip_dir)
downloader_t.download()
downloader_p = CMIPDownloader(var='pr', starty=1979, endy=2100, shape=catchment, processes=4, dir=cmip_dir)
