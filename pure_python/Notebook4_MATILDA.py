...
#     <b>Note:</b> Calibrating the model is computationally intensive and will be slow to run on a single CPU in the online environment. For a comprehensive calibration that takes full advantage of the <a href="https://spotpy.readthedocs.io/en/latest/" target="_blank">spotpy</a> library, we recommend downloading the notebooks and running them on a local machine with multi-core processing capabilities. Additional options to reduce calibration time are described in this Notebbook.
# </div>
#
